...
public class RepositoryCommon {
	...
	public List<com.shift102.model.Exception> getAllExceptions() {
		SqlRowSet srs = jdbcTemplate
				.queryForRowSet("SELECT EXCP_ID,EXCP_CODE FROM EXCEPTIONTYPES");
		List<com.shift102.model.Exception> excp= new ArrayList<com.shift102.model.Exception>();
		while (srs.next()) {
			excp.add(new com.shift102.model.Exception(srs.getInt("EXCP_ID"),srs.getString("EXCP_CODE")));
		}
