...
ns.view = (function() {
    ...
    return {
		...
		showExistingdataForEdit: function(shifts){
			...
			rows+=`<tr><td>Resource</td><td>Data&nbsp;Type</td>`;
            ...
            if (shifts) {
				...
				for (let i=0, l=userShifts.length; i < l; i++) {
					...
					rows+=`<td class="text-nowrap">`+userShifts[i].user_name+`</td>`;
					...
					for (let j=0, k=dates.length; j < k; j++){
								rows += `<td class="`+userShifts[i].excelData[j].excp_code+`"><small>`+userShifts[i].excelData[j].excp_code+`</small></td>`;
						}
						
					rows += `</tr>`;
					if(userShifts[i].typeOfData === 'A') rows+=`<tr></tr>`;
				}
